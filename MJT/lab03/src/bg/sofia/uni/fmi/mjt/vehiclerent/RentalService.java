...
public class RentalService {
     ...
     */
    public void rentVehicle(Driver driver, Vehicle vehicle, LocalDateTime startOfRent) throws IllegalArgumentException {
        if(driver == null || vehicle == null || startOfRent == null) {
            throw new IllegalArgumentException("Invalid argument(s) with value of null.");
        }

        if(vehicle.isRented()) {
            throw new VehicleAlreadyRentedException("Vehicle has already been rented.");
        }
    ...
    public double returnVehicle(Vehicle vehicle, LocalDateTime endOfRent) throws InvalidRentingPeriodException {
        if(endOfRent == null || vehicle == null) {
            throw new IllegalArgumentException("Vehicle or end time of renting cannot be null.");
        }

        if(!vehicle.isRented()) {
            throw new VehicleNotRentedException("Vehicle not yet rented, cannot be returned.");
        }

        double endPriceForRent = vehicle.calculateRentalPrice(vehicle.getStartRentTime(), endOfRent);

        vehicle.returnBack(endOfRent);

        return endPriceForRent;
    }
}
